...
No. Polling only checks the button at the single moment digitalRead() runs, once per pass through loop(). The 10000 serial prints take over two minutes at 9600 baud, so the button gets sampled about once every two minutes. A press that starts and ends inside that window is never seen — it isn't delayed, it's lost.
